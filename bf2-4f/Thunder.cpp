...
Thunder::Thunder(float x, float y, bool flg)
{
...
void Thunder::Reflection(const BoxCollider* boxcollider)
{
	...
	{
		//下
		if (this->GetMax().y > boxcollider->GetMin().y &&
			this->GetMin().y < boxcollider->GetMin().y)
		{
			angle = (1 - angle);
			ChangeAngle();
		}
		...
		//上
		if (this->GetMin().y < boxcollider->GetMax().y &&
		...
		this->GetMax().y > boxcollider->GetMin().y + 5)
	{
		...
		{
			location.x = boxcollider->GetMin().x - area.width - 2;
			angle = (1 - angle) + 0.5f;
			if (angle > 1)
			{
				angle -= 1.0f;
		...
		{
			location.x = boxcollider->GetMax().x + 2;
			angle = (1 - angle) + 0.5f;
			if (angle > 1)
			{
				angle -= 1.0f;
